...
| **Compilation control**| TRITON_SIMT_STACK_LIMIT | **1152** (CANN per-thread stack) | Sets the SIMT per-thread stack-size limit (bytes) forwarded to `bishengir-compile` as `--simt-stack-limit`. The per-kernel `simt_stack_limit` option takes precedence; this env var is the next fallback. A negative value disables the downstream check; `0` is a valid (strict) limit, not a disable sentinel. Accepts decimal or hex (e.g. `0x800`). | Integer (decimal or hex). | |
...
| **SIMT stack safety** | `simt_stack_limit` | Default `1152` (CANN per-thread stack) | Per-kernel SIMT per-thread stack-size limit (bytes) forwarded to `bishengir-compile` as `--simt-stack-limit`. Overrides `TRITON_SIMT_STACK_LIMIT`. Negative disables the downstream check; `0` is a valid (strict) limit, not a disable sentinel. | `triton.Config` or launch meta-parameter |
